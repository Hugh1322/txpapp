					...
					<div class="p25 goods_price">
						...
						<i class="f14"></i>
							<span class="J_price" style="display: block;">
								<c:forEach items="${product.productSku.buyScheduleSettingLs }" var="scheduleSetting" varStatus="status">
									${scheduleSetting.buyScheduleConfigName }&nbsp;:&nbsp;￥${scheduleSetting.packagePrice } 
									&nbsp;&nbsp;
									<span class="J_js" style="font-size: 12px;color: #000;">节省：￥${product.productSku.marketPrice - scheduleSetting.packagePrice }</span>
									<br>
								</c:forEach>
							</span>
							<c:forEach items="${product.productSkus }" var="productSku" varStatus="status">
								<c:if test="${productSku.productSkuId ne product.productSku.productSkuId }">
									<span class="J_price" style="display: none;">
										<c:forEach items="${productSku.buyScheduleSettingLs }" var="scheduleSetting" varStatus="status">
											${scheduleSetting.buyScheduleConfigName }&nbsp;:&nbsp;￥${scheduleSetting.packagePrice } 
											&nbsp;&nbsp;
											<span class="J_js" style="font-size: 12px;color: #000;">节省：￥${productSku.marketPrice - scheduleSetting.packagePrice }</span>
											<br>
										</c:forEach>
									</span>
								</c:if>
						...
						<br>
						<del>原价：<span class="f12">￥</span><span class="J_oPrice">${product.productSku.marketPrice }</span></del>
						
					</div>
					<div class="p25 goods_msg">
						<p class="goods_wi">重量：<span class="J_wi">${product.productSku.skuWeight }</span>g</p>
						<p class="goods_dress">产地：
						<c:forEach items="${product.propsMap['area'] }" var="prop"  >
						<c:if test="${status.index != 0 }"><span>|</span></c:if>${prop.name }
						</c:forEach>
						</p>
						<div class="cf goods_sku">
							<span>规格：</span>
							<ul class="cf J_sku_sched">
								<c:forEach items="${product.productSkus }" var="productSku">
								<li
								<c:choose>
									<c:when test="${productSku.inventory eq 0 }">class="disable"</c:when>
									<c:when test="${productSku.productSkuId eq product.productSku.productSkuId }">class="cur"</c:when>
								</c:choose>
									data-productSkuId="${productSku.productSkuId }" data-weight="${productSku.skuWeight }" data-price="${product.productSku.buyScheduleSettingLs }" data-oPrice="${productSku.marketPrice }" data-num="${productSku.inventory }">
