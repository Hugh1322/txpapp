				...
				<form id="goodsForm">
					...
					<div class="p25 goods_price">
						<c:forEach items="${product.productSkus }" var="productSku" varStatus="status">
								<c:if test="${not empty productSku.isDefault }">
									<c:set value="${productSku.isDefault}" var="default"/>
								</c:if>
						<c:if test="${status.index+1 eq default}">
						一品价：<span class="red">
						<i class="f14">￥</i><span class="J_price">${productSku.unitPrice }</span></span> 
						<br>
						<del>原价：<span class="f12">￥</span><span class="J_oPrice">${productSku.marketPrice }</span></del>
						节省：<span class="f12">￥</span><span class="J_js">${productSku.marketPrice - productSku.unitPrice }</span>
						</c:if>
						</c:forEach>
					</div>
					...
					<c:forEach items="${product.productSkus }" var="productSku" varStatus="status">
						...
						<c:if test="${status.index+1 eq default}">
						<p class="goods_wi">重量：<span class="J_wi">${productSku.skuWeight }</span>g</p>
						</c:if></c:forEach>
						...
						<div class="cf goods_sku">
							...
							<ul class="cf J_sku">
								<c:forEach items="${product.productSkus }" var="productSku" varStatus="status">
								<c:if test="${not empty productSku.isDefault }">
									<c:set value="${productSku.isDefault}" var="isDefault"/>
								</c:if>
								<li  
								<c:choose>
									<c:when test="${productSku.inventory eq 0 }">class="disable"</c:when>
									<c:when test="${status.index+1 eq isDefault}">class="cur"</c:when>
								</c:choose>
								...
								<c:if test="${status.index+1 eq productSku.isDefault}">
						...
						<div class="goods_tj">
							...
							<div class="goods_zc"> 支持：
								<c:if test="${product.productSku.payType==1}">
									在线支付
								</c:if>
								<c:if test="${product.productSku.payType==2}">
									在线支付    |  货到付款
								</c:if>
							</div>
