...
<div class="body m-w980p">
	...
	<input id="userIdKey" value="${product.userId}" type="hidden" >
	<input id="skuId" name="item.skuId" value="${ item.skuId}" type="hidden" >
	<input id="status" name="item.status" value="${ item.status}" type="hidden" >
	...
	<input name="item.productStatus" value="${ product.status}" type="hidden" >
		<table border="0" class="addPro-table">
			<tr>
				<th width="145" class="left-td">商品dd属性</th>
				<th width="130">&nbsp;</th>
				<th width="160">&nbsp;</th>
				<th width="543">&nbsp;</th>
			</tr>
			...
			<tr>
				<td class="left-td"><i class="red">*</i>商品名称：</td>
			...
			<tr>
				<td class="left-td"><i class="red">*</i>商品别名：</td>
				<td colspan="2"><input type="text" class="pro-name" name="item.productAlias" value="${item.productAlias}" id="aliasName"/></td>
...
// 	}
	...
	$("#aliasName").formValidator({validatorGroup: "gp",onFocus:"商品别名为1-16个字符组成"}).inputValidator({min:1,max:16,empty:{leftEmpty:false,rightEmpty:false,emptyError:"两边不能有空格"},onError:"商品别名为1-16个字符组成"}).regexValidator({ regExp: "^[\u4E00-\u9FA5A-Za-z0-9_\\s\(\)]+$", onError: "商品别名为1-16个字符组成" });
	...
	var queryProduct = function(brandNo){
		...
		zid = $("#zoneId").val();
		var urlPath = _ctxPath+"/seller/product/group/gp-listSkuManager.htm";
		var brand_temp = "p-list";
		alert('$("#userId").val():' + $("#userIdKey").val());
		$.ajax({
			url : urlPath,
			type : "POST",
			data :  {'firstChar' : brandNo, 'userId' : $("#userIdKey").val()},
			dataType : "html",
	...
	function subForm(url){
		...
		$.ajax({
			...
			success:function(msg){
				alert(msg.info);
				if(msg.code =="true"){
				   window.location.replace(_ctxPath + '/admin/product/product-searchProducts.htm');
				}
			},
			error: function(xhr){
