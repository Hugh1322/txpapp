...
<div class="body m-w980p">
	...
	<input id="skuId" name="item.skuId" type="hidden" >
	<input id="brandId" name="item.brandId" type="hidden" >
	<input id="area" name="item.area" type="hidden" >
	<input id="kind" name="item.kind" type="hidden" >
	<input id="spec" name="item.spec" type="hidden" >
	
		<table border="0" class="addPro-table" style="height: 100%">
			<tr>
						...
						<tr  >
							<td width="450" >
								<table class="skuTable" width="630" class="small">
									...
									<tr>
										<td  id="skuSpecification"> </td>
										<td id="p_weitht"> </td>
										<td id="inventory"> </td>
										<td >
										<input style="width: 50px" id="quantity" type="text"  name="item.inventory"  ></td>
										<td id="skuCode"> </td>
										<td ><input style="width: 50px" id="price" name="item.price" type="text"   ></td>
										<td id="unitPrice"> </td>
...
// 	}
		...
		$.ajax({
			...
			success : function(message) {
				var product = eval('('+message.info+')')
				 ...
				 $("#marketPrice").html(product.productSku.marketPrice);
 

				 editvar.setData(product.description);

				var i =0;
				for(i=0;i<product.imageList.length;i++){
					var $del_img = $(".no-border-table .del-img").eq(i);
					var $td = $del_img.closest("td");
					$td.find(".default-img").find("img").remove();
					$del_img.after("<img src='${_filePath }"+product.imageList[i]+"'/>");
					$td.find(".imageUrl").val(product.imageList[i]);
				}
	...
	$("#submitReview").click(function(){
		subForm( _ctxPath + '/seller/product/group/gp-saveAndCheck.htm');
	});
