...
<body>
	...
	<%--  <jsp:include page="../include/pageManageMenu.jsp"  flush="true" />  --%>
		 <form class="sub-form m-clear m-mt10p" action="${_ctxPath }/admin/adv/dailyBanner_toAddDetail.htm" id="addForm" method="post">
		 	<div class="m-fl"><%-- 广告类型 --%>
		          		 	<input type="submit" class="m-btn" value="新增通栏广告" />
	         </div>
                ...
                <tbody>
                 <c:forEach items="${advPage.result}" var="adv" varStatus="status">
                 	<tr>
						...
						<td>
							<div class="relative">
								<c:if test="${adv.picUrl != '' && adv.picUrl!= null}">
									<span style="display: none;"><yp:thumbImage originalPath='${adv.picUrl}' imageType='t5'></yp:thumbImage></span>
						...
						<td><a onclick="clickAddOrEditBanner('${adv.dailyBannerId}','${_ctxPath}/admin/adv/dailyBanner_toEditDetail.htm')" style="cursor: pointer;">编辑</a>&nbsp;
						<a style="cursor: pointer;" onclick="delBanner('${adv.dailyBannerId}','${_ctxPath}/admin/adv/dailyBanner_delDetail.htm')" class="m-5">删除</a></td>
					</tr>
                 </c:forEach>
                </tbody>
            </table>
	...
	function delBanner(id,url){// 删除广告
		$.dialog({
			  lock: true,
			  padding: "5px",
			  title:false,
