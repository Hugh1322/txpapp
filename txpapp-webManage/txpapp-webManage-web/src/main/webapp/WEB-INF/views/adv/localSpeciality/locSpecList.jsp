...
<body>
	...
	<%--  <jsp:include page="../include/pageManageMenu.jsp"  flush="true" />  --%>
		 <form class="sub-form m-clear m-mt10p" action="${_ctxPath }/admin/adv/locSpec_toAddDetail.htm" id="addForm" method="post">
		 	<div class="m-fl"><%-- 广告类型 --%>
		          		 	<input type="submit" class="m-btn" value="新增${position.positionName }" />
				<a style="cursor: pointer;" onclick="delAdv('${adv.advertisementId}','${_ctxPath}/admin/adv/locSpec_delDetail.htm')" class="m-5">删除</a>
                ...
                <tbody>
                 <c:forEach items="${advPage.result}" var="adv" varStatus="status">
                 	<tr>
						...
						<td> 
						<c:if test="${adv.rank == 1 }"> 左侧</c:if>
						<c:if test="${adv.rank == 2 }"> 中侧</c:if>
						<c:if test="${adv.rank == 3 }"> 右侧</c:if>
						</td>
						<td><a onclick="clickAddOrEdit('${adv.advertisementId}','${_ctxPath}/admin/adv/locSpec_toEditDetail.htm')" style="cursor: pointer;">编辑</a>&nbsp;
						<a style="cursor: pointer;" onclick="delAdv('${adv.advertisementId}','${_ctxPath}/admin/adv/locSpec_delDetail.htm')" class="m-5">删除</a></td>
					</tr>
                 </c:forEach>
                </tbody>
            </table>
