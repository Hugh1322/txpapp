...
<title>优惠券广告</title>
    ...
    <link href="${_jsPath }/plugin/artdialog/skins/coupon.css" rel="stylesheet" />
    ...
    <script src="${_jsPath}/pages/couponAdv.js" language="javascript"></script>
    <!--[if IE]>
...
<body>
	...
	<%--  <jsp:include page="../include/pageManageMenu.jsp"  flush="true" />  --%>
		 <form class="sub-form m-clear m-mt10p" action="${_ctxPath }/admin/adv/couponAdv_toAddDetail.htm" id="addForm" method="post">
		 	<div class="m-fl"><%-- 广告类型 --%>
...
<%-- 						<td><input type="text" data-default="${adv.rank} " name="adv.rank" value="${adv.rank }" id="hiddenText" style="width:80px;display:none;"/></td> --%>
						<td> 
							<div class="relative">
						...
						<td><a onclick="clickAddOrEdit('${adv.advertisementId}','${_ctxPath}/admin/adv/couponAdv_toEditDetail.htm')" style="cursor: pointer;">编辑</a>&nbsp;
						<a style="cursor: pointer;" onclick="delAdv('${adv.advertisementId}','${_ctxPath}/admin/adv/couponAdv_delDetail.htm')" class="m-5">删除</a></td>
					</tr>
                 </c:forEach>
                </tbody>
            </table>
		...
		$(".J_edit").on("click", function() {
			...
			if ($this.text() == "调整") {
				...
				if (rank == "") {
					...
					if (!reg.test(rank) || rank > 10 || rank < 1) {
						alert("请输入1-10的正整数");
						return false;
					}
				}
