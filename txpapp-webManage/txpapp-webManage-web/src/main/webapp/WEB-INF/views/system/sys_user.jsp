...
<script type="text/javascript">
	...
	function submitForm() {
		var startTime = $("#startTime_sysuser").val();
		var endTime = $("#endTime_sysuser").val();
		if (startTime && endTime) {
			if (startTime > endTime) {
				BJUI.alertmsg('error', "开始时间不能大于结束时间！");
				return;
			}
		}
		...
		// var form = $("#pagerForm");
		// form.submit();
		$("form[name='sysUserForm']").submit();
	}

</script>
<div class="bjui-pageHeader">
	<form id="pagerForm" name="sysUserForm" data-toggle="ajaxsearch"
		...
		<div class="bjui-searchBar">
			...
			<select onchange="$('.userStatus').val(this.options[this.selectedIndex].value);" data-toggle="selectpicker">
				<option value="">全部</option>
				<option value="0" <c:if test="${u.status==0}">selected="selected"</c:if> >禁用</option>
				<option value="1" <c:if test="${u.status==1}">selected="selected"</c:if> >启用</option>
			</select>&nbsp;
			<button type="button" class="showMoreSearch" data-toggle="moresearch"
					data-name="custom2">
				<i class="fa fa-angle-double-down"></i>
			</button>
			<button type="button" class="btn-default" data-icon="search" onclick="submitForm();">查询</button>
		...
		<div class="bjui-moreSearch">
			<label>创建时间：</label><input type="text" id="startTime_sysuser"
									   value='${beginTime}'
									   name="beginTime" size="20" data-toggle="datepicker"
									   data-pattern="yyyy-MM-dd" />—<input type="text"
																		   id="endTime_sysuser" name="endTime" size="20"
																		   value='${endTime}'
...
<div class="bjui-pageFooter">
	...
	<div class="pagination-box" data-toggle="pagination"
		 ...
		 data-page-current="${page.pageNo}">aa
	</div>
</div>