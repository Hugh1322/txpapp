 ...
 */
function tinyImgUpload(ele, options, width_, height_) {
    // 判断容器元素合理性并且添加基础元素
    ...
    if(eleList.length == 0){
        ...
        eleList[0].innerHTML ='<div id="img-container" >'+
            '<div class="img-up-add  img-item"> <span class="img-add-icon">添加</span> </div>'+
            //'<input type="file" name="files" id="img-file-input" accept="image/*" multiple>'+
                    ...
                    return function (e) {
                        ...
                        image.onload=function(){
                            ...
                            if(width <= width_ || height <= height_) {
                                ...
                                //控制只能加一张图片，加完后隐藏
                                $(".img-add-icon").hide();
                                $("#img-file-input").attr("disabled","disabled");
                            }
                            else {
                                // 图片文件绑定到容器元素上  这一行代码从上面移动下来 条件满足后才push
                                ele.files.pop();
                                $.dialog({
                                    title: false,
                                    content: "所选图片不满足条件！请重新选择",
                                    time : 2000
                                });
                                //alert("所选图片不满足条件！请重新选择");
                            }
                        };
                        //end -------------------------------------这段代码是验证图片的像素
                    };
    ...
    function removeImg(evt) {
        if(evt.target.className.match(/img-remove/)){
            ...
            //删除图片 回显可以继续添加图片
            $(".img-add-icon").show();
            $("#img-file-input").removeAttr("disabled");
        }
    }
