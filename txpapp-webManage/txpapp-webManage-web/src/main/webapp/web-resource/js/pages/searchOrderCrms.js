...
// 滚动条到底部，显示最新消息
var scrollBottom = function(orderId){
  var $li = $("#msg-main_"+orderId+" li:last"),
  		thisTop =0;
  if($li.length>0){
	thisTop = $li.position().top + $("#msg-main_"+orderId).scrollTop();
	$("#msg-main_"+orderId).scrollTop(thisTop);
  }
}
//点击回车提交客服内容
...
/**------------新的添加客服留言js方法-------------*/
var saveOrderCrm2 = function(orderId){
	var userName = '',
	myDate = new Date();
	myDate = ChangeTimeToString(myDate);
	var temp = HTMLEncode($("#remark_"+orderId).val());
	$("#orderIdCrm_"+orderId).val(orderId);
	if(temp==null || temp.length==0)
	{
		$("#remarkTip_"+orderId).html("内容不能为空");return false;
	}
	else if(temp.length>500)
	{
		$("#remarkTip_"+orderId).html("请输入500字符以内的内容");return false;
	}
	else{
		$("#remarkTip_"+orderId).html("");
		$.ajax({
			url:_ctxPath + "/admin/orderCrm/orderCrm-addOrderCrm.htm",
			type:"POST",
		...
		var temp_html = "<li><p style=\"padding:0;\"><b>"+userName+"</b><span> "+myDate+"</span></p><p style=\"padding:0;\">"+temp+"</p></li>";
		$(".msg-main2 ul").append(temp_html);
		// 保存信息之后清空输入框
		$(".J-txt").val("");
		// 滚动条到最底部，显示最新记录的信息
		scrollBottom();
