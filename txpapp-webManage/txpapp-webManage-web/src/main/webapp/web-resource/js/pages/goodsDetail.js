$(function() {
	...
	//分期送商品--规格切换
	yipin.skuSchedule=function(){
		$(".J_sku_sched li:not('.disable')").on("click",function(){
			var $this=$(this);
			...
			var priceSpan=$('.J_price').eq(index);
			priceSpan.show().siblings('span').hide();
	...
	yipin.form=function(){
		...
		//抢购立即购买
		$(".w_norm,.suspend").on("click",'.J_submitQg',function(){
			$("#goodsForm").submit();
		});
		$(".suspend").on("click",'.J_submitQg',function(){
			$("#goodsForm").submit();
		});
	};
	//计算运费
	yipin.yunf=function(val){
		var weight=$(".J_sku .cur").attr("data-weight");
		var num=0;
		num=yipin.multiply(weight,val);
		if(num>1000){
			$(".J_yf").text(20);
			$(".J_yh").text(20);
		}else{
	...
	yipin.init = (function() {
		...
		$.ajax({
			"url":_ctxPath+"/updateProductHits.htm",
			"type":"post",
			"dataType":"json",
			"data":{
		...
		if($(".J-countdown").length>0){
			$(".J-countdown").each(function () {
				var $this = $(this),
					_parent = $this.closest('.J_cd_Detail');
				data = $this.attr('data-config');
				data = eval('(' + data + ')');
				//活动开始回调
				data.callbackOne = function(){
					if(parseInt($(".J_sku .cur").attr("data-num")) > 0){
						_parent.find(".goods_form>a").removeClass().addClass("redBtn J_submitQg").text('立即抢购');
						_parent.find(".pr_btn>a").removeClass().addClass("redBtn1 J_submitQg").text('立即抢购');
					}
				};
				//活动结束回调
				data.callbackTwo = function(){
					_parent.find(".goods_form>a").removeClass().addClass("grayBtn").text('已过期');
					_parent.find(".pr_btn>a").removeClass().addClass("grayBtn1").text('已过期');
				};
