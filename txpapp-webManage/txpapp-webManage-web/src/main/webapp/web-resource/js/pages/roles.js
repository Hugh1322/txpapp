$(function(){
		...
		$.ajax({
			...
			success : function(data) {
				...
				for(var i=0;i<allUresouceListMenu.length; i++){
						if(allUresouceListMenu[i].url=="/admin/user/user-editPassword"||allUresouceListMenu[i].url=="/order/order-myOrders"||
								allUresouceListMenu[i].url=="/order/order-myOrdersAjax"||allUresouceListMenu[i].url=="/order/order-myOrderDetail"||
								allUresouceListMenu[i].url=="/address/address-getUserAddress"||allUresouceListMenu[i].url=="/address/address-getSingleReceiverAddress"||
								allUresouceListMenu[i].url=="/address/address-addReceiverAddress"||allUresouceListMenu[i].url=="/address/address-delReceiverAddress"||
								allUresouceListMenu[i].url=="/address/address-setDefualtReceiverAddress"||allUresouceListMenu[i].url=="/order/order-myRefundManage"||
								allUresouceListMenu[i].url=="/order/order-myRefundRecord"||allUresouceListMenu[i].url=="/user/user-checkPwdVaild"||
								allUresouceListMenu[i].url=="/user/user-changePsw"||allUresouceListMenu[i].url=="/user/user-updatePassword"||allUresouceListMenu[i].url=="/user/points"||
								allUresouceListMenu[i].url=="/suggest/ajaxSaveSuggest"||allUresouceListMenu[i].url=="/user/points"||
								allUresouceListMenu[i].url=="/suggest/searchSuggest-1"||allUresouceListMenu[i].url=="/suggest/searchSuggest-0"||
								allUresouceListMenu[i].url=="/user/user-userInfo"||allUresouceListMenu[i].url=="/user/user-checkEmailVaild"||
								allUresouceListMenu[i].url=="/order/order-getOrderInfo"||allUresouceListMenu[i].url=="/order/order-submitOrder"||
								allUresouceListMenu[i].url=="/netpay/payit"||allUresouceListMenu[i].url=="/netpay/confirmOrder"||
								allUresouceListMenu[i].url=="/order/order-panicBuying"||allUresouceListMenu[i].url=="/order/order-showCart"||
								allUresouceListMenu[i].url=="/order/order-myOrderCancelManage"||allUresouceListMenu[i].url=="/order/order-myOrderCancelRecord"
									||allUresouceListMenu[i].url=="/seller/proshare/cashRecord-ajaxApplyForCash"){//默认就给予的url 修改密码与前台url
								$(".pop-box-left").append('<div style="display:none;" id="div'+allUresouceListMenu[i].uresourceId+'" class="role-list"><i></i><p id="'+allUresouceListMenu[i].uresourceId+'" class="J-parent"><em class="sel" ></em><label title="'+allUresouceListMenu[i].uresourceId+'">'+allUresouceListMenu[i].uresourceName+'</label></p><ul></ul></div>');
						}else{
	...
	$("#addKeyword").on("click",function(){
			...
			var userName = $("input[name='myUser.username']").val()
