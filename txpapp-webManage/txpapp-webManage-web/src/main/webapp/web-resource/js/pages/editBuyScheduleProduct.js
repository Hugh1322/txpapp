...
//		}
		...
		$.ajax({
			type:'POST',
			url: _ctxPath + '/seller/product/buySchedule-saveProduct.htm',
...
//		}		
		...
		$.ajax({
			type:'POST',
			url: _ctxPath + '/seller/product/buySchedule-submitReview.htm',
			data: $("#addForm").serialize()+weightData,
			success:function(msg){
				if(msg.info == "true"){
					...
					$(".tip-main").show();
					if(shfz){
						$("#addForm").attr("action", _ctxPath + "/seller/product/buySchedule-doCopyProductSku.htm");
	...
	$(".input-checkbox").click(function(){
		$.each($(".repertories").find("tr:gt(0)"), function(i,tr){
			var $tr = $(tr);
			var unitPriceAry = new Array();
			$tr.find(".unitPrice").each(function(i,td){
				unitPriceAry[i]=$(td).val();
			})
			var value = {
				"id" : $tr.attr("id"),
...
//				"unitPrice" : $tr.find(".unitPrice").val(),
				"scheduleUnitPriceLs" : unitPriceAry,
...
function createRepertoriesTable(){
	...
	//分期送计划配置选项数
	var buyScheduleConfigCount = $("#buyScheduleConfigCount").val();
	//没勾选一个选项时
	if($sizes.length == 0){
		variableInit();
		$tr_productSku.attr("id", "_");
		//分期送价格input
		var stagingSendUnitPriceLsStr="";
		for (var i=0;i<buyScheduleConfigCount;i++)
		{
			stagingSendUnitPriceLsStr=stagingSendUnitPriceLsStr+"<td><input type='text' class='repertories-num unitPrice' maxlength='11' name='product.productSkus[0].scheduleUnitPriceLs["+i+"]' onchange='checkUnitPrice(this)'/></td>";
		}
	...
	//仅勾选了尺寸时
		...
		$.each($sizes, function(i,size){
			...
			for (var i=0;i<buyScheduleConfigCount;i++)
			{
				stagingSendUnitPriceLsStr=stagingSendUnitPriceLsStr+"<td><input type='text' class='repertories-num unitPrice' maxlength='11' name='product.productSkus["+index+"].scheduleUnitPriceLs["+i+"]' onchange='checkUnitPrice(this)'/></td>";
			}
...
//			alert(obj.scheduleUnitPriceLs);
			//设置分期送各期价格 start 徐国飞
			var scheduleUnitPriceAry = eval(obj.scheduleUnitPriceLs);
			if(scheduleUnitPriceAry!=null&&scheduleUnitPriceAry.length>0){
				$($input_unitPrice).each(function(index,data){
					$(this).val(scheduleUnitPriceAry[index]);
				});
			}
			
			//设置分期送各期价格 end 徐国飞
			//设置市场价
			var $input_marketPrice = $productSku_tr.find(".marketPrice");
			$input_marketPrice.val(obj.marketPrice);
		});
	}
	...
	$.each($(".repertories").find("tr:gt(0)"), function(i,tr){
		...
		var value = {
			...
			"skuCode" : $tr.find(".skuCode").val(),
			//"unitPrice" : $tr.find(".unitPrice").val(),
			"scheduleUnitPriceLs" : $tr.find(".unitPrice").val(),
			"marketPrice" : $tr.find(".marketPrice").val(),
		};
		data.push(value);
	});
