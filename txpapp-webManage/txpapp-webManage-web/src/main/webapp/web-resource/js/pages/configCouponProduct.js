$(function() {
	...
	$(".select-all-sku").on('click', function() {
		...
		if ($thisEm.hasClass("sel")) {
			...
			$thisEm.removeClass("sel");
			 ...
			 doFormSubmit(_ctxPath+'/admin/coupon/config-removeAllProductInActivity.htm');
	...
	$(".select-sub-sku").on('click',function() {
		...
		if ($thisEm.hasClass("sel")) {
			...
			$thisEm.addClass("sel");
			doFormSubmit(_ctxPath+'/admin/coupon/config-addProductToActivity.htm');
		}
	});
	// 修改活动价
	...
	$(".activityPrice").on('blur',function() {
		...
		doFormSubmit(_ctxPath+'/admin/coupon/config-updateProductSettlementPrice.htm');
	});
	...
	$("#activity_discount_but").on('click',function() {
		...
		setProductSkuIds();
		doFormSubmit(_ctxPath+'/admin/coupon/config-updateAllProductSettlementPrice.htm');
	});
