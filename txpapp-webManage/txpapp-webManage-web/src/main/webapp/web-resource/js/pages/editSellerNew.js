$(function(){
	...
	var nation = $("#company_area_nation").val(), prov = $("#company_area_prov").val(), city = $("#company_area_city").val(), district = $("#company_area_region").val();
	var linkage = new linkageSelect();
	 linkage.init({
	     "zeroSel":['#company_nation','请选择',nation],
	     "oneSel":['#company_province','请选择',prov],
		 "twoSel":['#company_city', '请选择',city],
		 "threeSel":['#company_region', '请选择',district]
	 });
     var nation1 = $("#shiper_area_nation").val(), prov1 = $("#shiper_area_prov").val(), city1 = $("#shiper_area_city").val(), district1 = $("#shiper_area_region").val();
	 var linkage1 = new linkageSelect();
     linkage1.init({
         "zeroSel":['#shiper_nation','请选择',nation1],
         "oneSel":['#shiper_province','请选择',prov1],
         "twoSel":['#shiper_city', '请选择',city1],
         "threeSel":['#shiper_region', '请选择',district1]
     });
     var nation2 = $("#receiver_area_nation").val(), prov2 = $("#receiver_area_prov").val(), city2 = $("#receiver_area_city").val(), district2 = $("#receiver_area_region").val();
	 var linkage2 = new linkageSelect();
     linkage2.init({
         "zeroSel":['#receiver_nation','请选择',nation2],
...
//		function allEmpty(val,elem){return ($("#alipay").val()=="" && $("#bank").val()=="")?'为了保证您能收到货款，请至少填写一种收<br/>款帐号！':true;} 
	...
	//验证三级联动
	$("#add-f").find("#shiperRegionCodesTemp").formValidator({
        onFocus:"请选择具体发货地址",
        tipID:"addrTip-f"
    }).inputValidator({
    	min:2,
	...
	$("#add-t").find("#receiverRegionCodesTemp").formValidator({
        onFocus:"请选择具体退货地址",
        tipID:"addrTip-t"
    }).inputValidator({
    	min:2,
...
//		if($("#shiperRegionCodesTemp").val()== '' || $("#shiperRegionCodesTemp").val() == 0) {
//			alert("请选择具体发货地址");
//			return;
//		}
//		if($("#receiverRegionCodesTemp").val()=='' && $("#receiverRegionCodesTemp").val() == 0) {
//			alert("请选择具体退货地址");
//			return;
//		}
        ...
        $('#companyRegionCodes').val($("#companyRegionCodesTemp").val());
        $('#shiperRegionCodes').val($("#shiperRegionCodesTemp").val());
        $('#receiverRegionCodes').val($("#receiverRegionCodesTemp").val());
